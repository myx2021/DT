...
class Annealer(object):
    ...
    best_score = float('inf')  # choose the largest number as the initial best score
    start = None
    ...
    def save_state(self, fname=None):
        ...
        if not fname:
            ...
            fname = date + "_energy_" + str(self.evaluation()) + ".state"
        ...
        while step < self.steps and not self.user_exit:
            ...
            if dE > 0.0 and math.exp(-dE / T) < random.random():
                ...
                if E < self.best_energy:
                    self.best_solution = self.copy_state(self.state)
                    self.best_evaluation = E
            if self.updates > 1:
